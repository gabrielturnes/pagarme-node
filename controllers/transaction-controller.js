...
transactionController.prototype.post = async (req, res) => {
  try {
    ...
    _validationContract.isRequired(req.body.cpf, "Informe o CPF");
    const data = req.body;
    ...
    if (data.card_id) {
      ...
      let transactionCreated = await _repo.create(transaction);
      ...
      const bytes = CryptoJS.AES.decrypt(data.card_hash, 'hdfudhuidfhudhudah9d8s8f9d8a98as9d8s9d89as');
